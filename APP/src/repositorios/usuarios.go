...
func (repositorio Usuarios) Buscar(nomeOuNick string) ([]modelos.Usuario, error) {
	...
	linhas, erro := repositorio.db.Query("select id,nome,nick,email,createdAt from usuarios where nome LIKE ? or nick LIKE ?", nomeOuNick, nomeOuNick)
	if erro != nil {
		return nil, erro
	}
	defer linhas.Close()
	...
	for linhas.Next() {
		...
		if erro = linhas.Scan(&usuario.ID, &usuario.Nome, &usuario.Nick, &usuario.Email, &usuario.CreatedAt); erro != nil {
			return nil, erro
		}
...
// BuscarPorID tras um usuario do banco de dados
func (repositorio Usuarios) BuscarPorID(ID uint64) (modelos.Usuario, error) {
	linhas, erro := repositorio.db.Query(
		"select id,nome,nick,email,createdAt from usuarios where id = ?",
		ID,
	)
	if erro != nil {
...
func (repositorio Usuarios) BuscarSeguindo(usuarioID uint64) ([]modelos.Usuario, error) {
	linhas, erro := repositorio.db.Query(`
		select u.id, u.nome, u.nick, u.email, u.createdAt from usuarios u inner join seguidores s on u.id = s.usuario_id where s.seguidor_id = ?
	`, usuarioID)
	if erro != nil {
		return nil, erro
	}
